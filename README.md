...
- **Bake definition:** `generated/docker-bake.hcl` (generated by `scripts/generate-bake.sh`)
- **Profiles folder:** `profiles/` (definitions) and `generated/profiles/` (expanded)
...
- **Feature**: a small, focused unit (folder under `features/<name>`) containing `feature.json` (metadata) and an `install.sh` (idempotent install actions). Features should be reusable and side-effect controlled.
- **Profile**: a named collection of features (defined in `profiles/matrix/*.yaml` and expanded to `generated/profiles/PROFILE-NAME`) that defines a staged image configuration (for example `base`, `dev`, `data-science`). Profiles are generated from YAML matrices for maintainability.
