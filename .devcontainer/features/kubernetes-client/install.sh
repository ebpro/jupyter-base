...
if [ -f "${PWD}/Artefacts/features/kubernetes-client/versions.json" ] || [ -f "${PWD}/Artefacts/versions.json" ] || [ -f /tmp/versions.json ]; then
  resolve_version() {
    local tool="$1" v=""
    if [ -f "${PWD}/Artefacts/features/kubernetes-client/versions.json" ]; then
      v=$(jq -r --arg t "$tool" '.tools[$t] // empty' "${PWD}/Artefacts/features/kubernetes-client/versions.json" 2>/dev/null || true)
      [ -n "$v" ] && { echo "$v"; return 0; }
    fi
