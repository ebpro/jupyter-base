...
resolve_version() {
  ...
  if [ -f "/tmp/Artefacts/features/typescript/versions.json" ]; then
    local ver=$(jq -r ".tools[\"${tool}\"] // empty" "/tmp/Artefacts/features/typescript/versions.json" 2>/dev/null || true)
