...
By placing this rule file in `.continue/rules/continue-codebase-access.md` you grant the `Continue` extension read-only access scoped to `#codebase` and subject to the restrictions above. If you want stricter limits (narrower paths, temporary access window), reply with the exact paths and duration and I'll update the rule accordingly.
